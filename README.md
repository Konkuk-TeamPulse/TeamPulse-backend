# TeamPulse Backend

Spring Boot 기반 TeamPulse 백엔드. 회원가입, 프로젝트, 태스크, 회의록, 초대, 리포트, 리스크 API를 제공한다.

- 운영 배포: https://teampulse-api.duckdns.org
- Frontend repo: https://github.com/Konkuk-TeamPulse/TeamPulse-frontend
- 운영 상태 확인: `GET /api/health` -> `status=UP`, `storageMode=mysql`

---

## 실행 환경

- Java 21 이상
- Maven Wrapper (`./mvnw` 또는 `.\mvnw.cmd`) 포함
- 운영 DB: MySQL (AWS RDS), 로컬 데모는 인메모리

## 환경변수 (.env)

운영 또는 로컬에서 MySQL 프로필로 실행할 때 다음 변수를 정의한다. `.env.example` 참고.

| 변수 | 설명 |
|---|---|
| `PORT` | 서버 포트 (기본 8080) |
| `SPRING_PROFILES_ACTIVE` | `mysql` (운영) / `demo` (로컬 인메모리) |
| `SPRING_DATASOURCE_URL` | `jdbc:mysql://<host>:3306/teampulse?useSSL=true&serverTimezone=Asia/Seoul&characterEncoding=UTF-8` |
| `SPRING_DATASOURCE_USERNAME` | DB 사용자 |
| `SPRING_DATASOURCE_PASSWORD` | DB 비밀번호 |
| `APP_CORS_ALLOWED_ORIGINS` | 콤마 구분 origin 목록 (운영 프론트 도메인 포함) |
| `APP_FRONTEND_PUBLIC_BASE_URL` | 초대 링크 생성용 프론트 base URL |

비밀값은 commit 하지 않는다. 운영 환경에서는 GitHub Actions/EC2 시크릿으로 주입한다.

### `.env` 작성 방법

저장소에는 실제 DB 주소, 계정, 비밀번호를 올리지 않는다. MySQL 프로필로 실행해야 할 때만 `.env.example`을 복사해서 로컬 전용 `.env`를 만든다.

```powershell
Copy-Item .env.example .env
```

그 다음 `.env` 안의 `<db-host>`, `<db-name>`, `<db-user>`, `<db-password>`, `<frontend-domain>` 값을 실제 실행 환경에 맞게 채운다. `.env`는 개인 실행 환경 파일이므로 commit 대상이 아니다.

## 로컬 실행

### Git clone 받은 경우

```powershell
git clone https://github.com/Konkuk-TeamPulse/TeamPulse-backend.git
cd TeamPulse-backend
git pull origin main
```

빠른 확인은 DB가 필요 없는 데모 프로필로 실행한다.

```powershell
.\mvnw.cmd spring-boot:run "-Dspring-boot.run.profiles=demo"
```

브라우저 또는 API 클라이언트에서 `http://localhost:8080/api/health`가 응답하면 실행 성공이다.

### ZIP 파일로 받은 경우

압축 해제 후 `TeamPulse-backend` 폴더에서 PowerShell을 열고 동일하게 Maven Wrapper를 실행한다.

```powershell
.\mvnw.cmd spring-boot:run "-Dspring-boot.run.profiles=demo"
```

ZIP에는 `.git` 정보가 없으므로 `git pull`은 생략한다.

### 운영(MySQL) 프로필

```powershell
$env:SPRING_PROFILES_ACTIVE="mysql"
$env:SPRING_DATASOURCE_URL="jdbc:mysql://<host>:3306/teampulse?useSSL=true&serverTimezone=Asia/Seoul"
$env:SPRING_DATASOURCE_USERNAME="<user>"
$env:SPRING_DATASOURCE_PASSWORD="<password>"
.\mvnw.cmd spring-boot:run
```

MySQL 프로필은 DB 접속 정보가 반드시 필요하다. 환경변수가 빠지면 서버가 정상 부팅되지 않는다.

### 데모(인메모리) 프로필

```powershell
.\mvnw.cmd spring-boot:run "-Dspring-boot.run.profiles=demo"
```

데모 프로필은 인메모리 저장이며 재시작 시 데이터가 초기화된다.

## 테스트

```powershell
.\mvnw.cmd test "-Dspring.profiles.active=demo"
```

JUnit/MockMvc 테스트가 모두 통과해야 PR을 merge한다. 테스트 개수는 변경될 수 있으므로 Maven 출력에서 `Failures: 0`, `Errors: 0`를 확인한다.

## 배포

- main 브랜치 push -> GitHub Actions(`.github/workflows/deploy-ec2.yml`)가 EC2 인스턴스에 SSH 접속하여 빌드/재기동.
- EC2 서버의 `.env`에 필수 환경변수가 없으면 기존 서버를 중단하기 전에 배포를 실패 처리한다.
- 배포 직후 `curl http://localhost:8080/api/health`가 30회까지 재시도로 헬스체크.
- main merge 후 GitHub Actions가 초록색으로 완료됐는지 확인한다.
- 배포 완료 약 1분 뒤 `https://teampulse-api.duckdns.org/api/health`에서 `status=UP`, `storageMode=mysql`을 확인한다.
- Actions가 빨간색이면 운영 서버 상태와 실패 로그를 먼저 확인한 뒤 다음 PR을 진행한다.

## 주요 API

명세형 API 응답은 `isSuccess`, `responseCode`, `responseMessage`, `result` wrapper를 사용한다.

- 인증: `POST /api/auth/signup`, `/login`, `/logout`
- 프로젝트: `POST/GET/PATCH /api/projects`, `GET /api/projects/{id}/dashboard`
- 태스크: `POST/GET /api/projects/{id}/tasks`, `PATCH/DELETE /api/tasks/{id}`, `PATCH /api/tasks/{id}/status`
- 의존관계: `POST /api/tasks/{id}/dependencies`, `DELETE /api/tasks/{id}/dependencies/{dependencyId}`
- 회의록: `POST/GET /api/projects/{id}/meetings`, `GET /api/meetings/{meetingId}`
- 초대: `POST /api/projects/{id}/invitations`, `GET/POST /api/invitations/{code}`
- 리포트: `POST /api/projects/{id}/reports`, `GET /api/reports/{id}/download`
- 리스크: `GET /api/projects/{id}/risks`
- 헬스: `GET /api/health`

## 디렉터리 구조

- `auth.api` 인증 컨트롤러
- `auth.application` 인증 서비스와 로그인 시도 제한
- `auth.infrastructure` 토큰 발급/검증 구현
- `auth.persistence` 인증 사용자와 refresh token JPA 저장소
- `workspace.api` 프로젝트, 태스크, 회의록, 초대, 리포트 API 컨트롤러
- `workspace.application` UseCase 인터페이스와 서비스 구현
- `workspace.dto` 요청/응답 record
- `workspace.persistence` 워크스페이스/태스크/회의록/팀원/리포트 JPA 엔티티와 Repository
- `project.api` 프로젝트 팀원 관리 API
- `common` 공통 응답, 보안 설정, 예외 처리
- `domain` 핵심 도메인 enum

기존 MVP 시절의 `/api/mobile/**`, `/api/account/**`, `/api/roadmap`, `/invite-links` 계열은 현재 공식 공개 API로 사용하지 않는다.
