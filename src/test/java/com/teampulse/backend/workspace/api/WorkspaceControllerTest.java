...
class WorkspaceControllerTest {
    ...
    void invitationApisUseNotionPathsAndSpecResponseShape() throws Exception {
        ...
        assertThat(inviteCode).hasSizeGreaterThanOrEqualTo(16);
        ...
        mockMvc.perform(patch("/api/projects/1")
                        ...
                        .content("""
                                {
                                  "projectName": "Member Cannot Update",
                                  "subject": "Forbidden",
                                  "description": "Member role must not update project",
                                  "startDate": "2026-04-02",
                                  "endDate": "2026-06-10"
                                }
                                """))
    ...
    void invalidInvitationLookupIsRateLimited() throws Exception {
        ...
        for (int attempt = 0; attempt < 20; attempt++) {
            mockMvc.perform(get("/api/invitations/{inviteCode}", invalidCode))
                    .andExpect(status().isBadRequest())
                    .andExpect(jsonPath("$.isSuccess").value(false));
        }
        ...
        mockMvc.perform(get("/api/invitations/{inviteCode}", invalidCode))
                ...
                .andExpect(jsonPath("$.isSuccess").value(false))
                .andExpect(jsonPath("$.responseCode").value(3010));
    }
